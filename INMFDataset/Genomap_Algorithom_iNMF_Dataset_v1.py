...
def load_and_prepare_data():
    """Loads and performs basic preprocessing on the PBMC dataset."""
    try:
    ...
    # Filter out cells with no gene counts
    sc.pp.filter_cells(adata, min_genes=1)

    # Basic preprocessing before batch correction
    sc.pp.normalize_total(adata, target_sum=1e4)
    sc.pp.log1p(adata)

    # --- FIX: Correctly handle NaNs in sparse or dense matrices ---
    if scipy.sparse.issparse(adata.X):
        # If data is in a sparse matrix, check its .data attribute
        if np.isnan(adata.X.data).any():
            adata.X.data[np.isnan(adata.X.data)] = 0
    elif np.isnan(adata.X).any():
        # If data is a dense matrix, check it directly
...
def main():
    """Main function to run the GenoMap analysis workflow using Scanorama."""
    ...
    plot_filename_prefix = "Genomap_ScanoramaPipeline_iNMF_Dataset"
