"""
=============================================================================
Script Name:    iNMF_Algorithom_iNMF_Dataset.py
Author:         Ariana Rahman
Affiliation:    Arizona State University / Stanford University
Date:           January 2026
Description:    
    This script performs single-cell integration using iNMF (Non-negative Matrix Factorization)
    ...
    - Generates 'iNMF_Imbalance_Robustness.png'.
=============================================================================
"""
...
def compute_all_metrics(adata):
    ...
    try:
        silhouette = silhouette_score(adata.obsm['X_nmf'], adata.obs['leiden'])
    except:
        silhouette = 0.0
    ...
    if CALCULATE_BATCH_METRICS:
        try:
            ilisi = compute_lisi_python(adata.obsm['X_nmf'], adata.obs[BATCH_KEY], k=90)
        except Exception as e:
            logging.warning(f"iLISI failed: {e}")
        try:
            kbet_result = kBET(adata, batch_key=BATCH_KEY, label_key=CELLTYPE_KEY, type_="embed", embed="X_nmf", verbose=False)
...
def run_inmf_pipeline(adata):
    """Runs NMF integration pipeline."""
    # iNMF requires non-negative data
    if scipy.sparse.issparse(adata.X):
        min_val = adata.X.data.min()
        if min_val < 0: adata.X.data -= min_val
    else:
        min_val = adata.X.min()
        if min_val < 0: adata.X -= min_val
...
def run_non_overlapping_experiment(adata_raw):
    ...
    adata_neg = run_inmf_pipeline(adata_neg) 
...
def main():
    ...
    # --- 1. Full Integration ---
    logging.info("Running iNMF on full dataset...")
    adata = run_inmf_pipeline(adata_raw.copy())
    ...
    prefix = "iNMF_Algorithm_iNMF_Dataset"
    ...
    if RUN_ROBUSTNESS_ANALYSIS:
        ...
        df_imb.to_csv(os.path.join(OUTPUT_FOLDER, "iNMF_imbalance_results.csv"), index=False)
        
        plt.figure(figsize=(6,5))
        for m in ['ARI', 'Silhouette', 'iLISI', 'kBET']:
            plt.plot(df_imb['fraction'], df_imb[m], label=m, marker='o')
        plt.legend()
        plt.title("iNMF Robustness to Imbalance")
        plt.savefig(os.path.join(OUTPUT_FOLDER, "iNMF_Imbalance_Robustness.png"), dpi=300)
