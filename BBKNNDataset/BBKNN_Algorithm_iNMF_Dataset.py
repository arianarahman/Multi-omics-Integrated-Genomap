"""
=============================================================================
Script Name:    BBKNN_Algorithom_iNMF_Dataset.py
Author:         Ariana Rahman
Affiliation:    Arizona State University / Stanford University
Date:           January 2026
Description:    
    This script performs single-cell integration using BBKNN (Batch Balanced kNN)
    and evaluates its performance on PBMC datasets.

    Key Experiments:
    1. Full Integration & Visualization: UMAP/t-SNE, ARI, Silhouette.
    2. Batch Mixing Assessment: iLISI and kBET (computed on UMAP space).
    ...
    - Generates 'BBKNN_Imbalance_Robustness.png'.
=============================================================================
"""
...
def run_non_overlapping_experiment(adata_raw):
    ...
    adata_neg = run_bbknn_pipeline(adata_neg) 
...
def compute_all_metrics(adata):
    ...
    # BBKNN doesn't produce an integrated high-dim embedding.
    # We use X_pca for silhouette (as per previous scripts) or X_umap.
    # Using X_pca here implies checking if clusters align in PCA space (baseline).
    silhouette = silhouette_score(adata.obsm['X_pca'], adata.obs['leiden'])
    
    ilisi, kbet = 0.0, 0.0
    if CALCULATE_BATCH_METRICS:
        try:
            # BBKNN mixes the graph, which affects UMAP. We evaluate mixing on UMAP.
            ilisi = compute_lisi_python(adata.obsm['X_umap'], adata.obs[BATCH_KEY], k=90)
        except Exception as e:
            logging.warning(f"iLISI failed: {e}")
        try:
            kbet_result = kBET(adata, batch_key=BATCH_KEY, label_key=CELLTYPE_KEY, type_="embed", embed="X_umap", verbose=False)
...
def run_bbknn_pipeline(adata):
    """Runs BBKNN integration pipeline."""
    # BBKNN requires PCA first
    sc.tl.pca(adata, svd_solver='arpack')
    
    # Run BBKNN (updates neighbors graph directly)
    bbknn.bbknn(adata, batch_key=BATCH_KEY)
    
    # Clustering & UMAP (using the BBKNN graph)
    sc.tl.leiden(adata, resolution=0.5)
    sc.tl.umap(adata)
    sc.tl.tsne(adata) # TSNE might not use the graph unless specified, usually defaults to X_pca
...
def main():
    ...
    # --- 1. Full Integration ---
    logging.info("Running BBKNN on full dataset...")
    adata = run_bbknn_pipeline(adata_raw.copy())
    ...
    prefix = "BBKNN_Algorithm_iNMF_Dataset"
    plot_embedding(adata.obsm['X_umap'], adata.obs[CELLTYPE_KEY], "UMAP", f"UMAP_{prefix}", custom_colors=label_to_color_map)
    # TSNE for BBKNN is often just PCA-based unless configured to use neighbor graph, 
    # but we plot what is generated for consistency.
    ...
    if RUN_ROBUSTNESS_ANALYSIS:
        ...
        for frac in [1.0, 0.5, 0.25, 0.1]:
            ...
            ad_ds = run_bbknn_pipeline(ad_ds)
        ...
        df_imb.to_csv(os.path.join(OUTPUT_FOLDER, "BBKNN_imbalance_results.csv"), index=False)
        
        plt.figure(figsize=(6,5))
        for m in ['ARI', 'Silhouette', 'iLISI', 'kBET']:
            plt.plot(df_imb['fraction'], df_imb[m], label=m, marker='o')
        plt.legend()
        plt.title("BBKNN Robustness to Imbalance")
        plt.savefig(os.path.join(OUTPUT_FOLDER, "BBKNN_Imbalance_Robustness.png"), dpi=300)
